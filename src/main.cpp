...
void setup(bool first) {
  ...
  if (first) {
    ...
    // Button images
    button_start.setImages("img/buttons/button_start.png",
                           "img/buttons/button_start_hover.png");
    button_highscores.setImages("img/buttons/button_scores.png",
                                "img/buttons/button_scores_hover.png");
    button_quit.setImages("img/buttons/button_quit.png",
                          "img/buttons/button_quit_hover.png");
...
void game() {
  if (gameScreen == SPLASH) {
    ...
    // Click buttons
    if (collision(mouse_x, mouse_x, -1, 200, mouse_y, mouse_y, 0, 920) ||
        key[KEY_LEFT]) {
    ...
    } else if (collision(mouse_x, mouse_x, 250, 1050, mouse_y, mouse_y, 185,
                         785) ||
               key[KEY_ENTER]) {
  ...
  if (key[KEY_ESC]) {
    ...
    if (gameScreen == INGAME) {
      while (true) {
        ...
        textprintf_centre_ex(buffer, font, 640, 400, makecol(0, 0, 0), -1,
                             "Press escape again to go to the main menu.");
        textprintf_centre_ex(buffer, font, 640, 440, makecol(0, 0, 0), -1,
...
void draw(bool toScreen) {
  if (gameScreen == SPLASH) {
    ...
    // Titles
    if (scoreDifficulty == 4) {
      textprintf_centre_ex(buffer, font, 640, 100, makecol(0, 0, 0), -1,
                           "Easy");
    ...
    for (int i = 0; i < 10; i++) {
      ...
      textout_ex(buffer, font, name.c_str(), 400, (i * 50) + 200,
                 makecol(0, 0, 0), -1);

      name = scores[i][1];
      textout_right_ex(buffer, font, name.c_str(), 860, (i * 50) + 200,
                       makecol(0, 0, 0), -1);
    }
  } else if (gameScreen == LEVELSELECT) {
    // Background
    ...
    textprintf_ex(buffer, font, 20, 20, makecol(0, 0, 0), -1, "Moves:%i",
                  moves);
    ...
    if (cards.size() == 0) {
      ...
      textout_ex(buffer, font, edittext.c_str(), 410, 410, makecol(0, 0, 0),
                 -1);

      // Draw the caret
      vline(buffer, text_length(font, edittext.c_str()) + 410, 412, 448,
            makecol(0, 0, 0));
    }
  }
